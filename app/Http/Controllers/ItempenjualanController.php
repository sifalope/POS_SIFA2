...
class ItemPenjualanController extends Controller
{
    ...
    public function destroy(ItemPenjualan $itempenjualan)
    {
        ...
        DB::transaction(function () use ($itempenjualan) {
            ...
            // Ambil relasi stok & penjualan sebelum item dihapus
            $produk = $itempenjualan->produk;
            $sale   = $itempenjualan->penjualan;

            // 1. Kembalikan stok jika produk ditemukan
            if ($produk) {
                $produk->increment('stok', $itempenjualan->kuantitas);
            }

            // 2. Hapus item penjualan
            $itempenjualan->delete();

            // 3. Update total penjualan (mencegah error jika bernilai null)
            if ($sale) {
                $totalSisa = $sale->itemPenjualan()->sum('subtotal') ?? 0;
                $sale->update([
                    'total_pembayaran' => $totalSisa
                ]);
            }
        });
